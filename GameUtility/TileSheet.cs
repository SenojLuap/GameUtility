...
namespace paujo.GameUtility {
  public class TileSheet {
    ...
    public virtual void Draw(SpriteBatch spriteBatch, Vector2 pos, int frame, float depth = 0f) {
      Texture2D texture = GetTexture(TextureKey);
      if (texture != null) {
	Rectangle srcRect = GetSourceRectangle(frame);
	spriteBatch.Draw(texture, position: pos, sourceRectangle: srcRect, color: Color.White, layerDepth: depth);
      }
    }


    public virtual void Draw(SpriteBatch spriteBatch, int x, int y, int frame, float depth = 0f) {
      Draw(spriteBatch, new Vector2((float)x, (float)y), frame, depth);
    }


    public virtual void Draw(SpriteBatch spriteBatch, float x, float y, int frame, float depth = 0f) {
	Draw(spriteBatch, new Vector2(x, y), frame, depth);
    }
